...
export class CarService {
  ...
  getCarsByBrand(brandId:number):Observable<ListResponseModel<CarDto>> {
   ...
   getCarsByColorByBrand(colorId:number,brandId:number):Observable<ListResponseModel<CarDto>> {
    let newPath=this.apiUrl+"cars/getcarsbycoloridbybrandid?colorId="+colorId+"&brandId="+brandId;
    return this.httpClient.get<ListResponseModel<CarDto>>(newPath);
